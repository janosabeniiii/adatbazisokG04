...
WHERE Darab > 1 //WITH+WHERE ugy mukodik mint a HAVING
RETURN Nev, Darab
```
...
```js
MATCH (p:Person)-[:ACTED_IN]->(m:Movie)<-[:DIRECTED]-(p:Person) //ket kapcsolat egy filmhez = ket iranybol kapcsoljuk
RETURN p.name
//tobb kapcsolat eseten lehet siman tobb MATCH utasitast irni (annyi ahany kapcsolat van)
//meg egy modszer: 

//MATCH (p:Person)-[:ACTED_IN]->(m:Movie)
//WHERE EXISTS ((p:Person)-[:DIRECTED]->(m:Movie)) -->ezt tagadni is lehet (NOT EXISTS)
```
...
ORDER BY COUNT(p.name) DESC, m.name
LIMIT 10
```
